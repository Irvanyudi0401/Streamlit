...
if menu == "Home":
    ...
    <div style='text-align: center; padding-top: 30px'>
        <h1>👁️ Deteksi Penyakit Mata Menggunakan Citra Fundus Retina</h1>
        <p style='font-size: 18px;'>
            Selamat datang di aplikasi <strong>Deteksi Penyakit Mata</strong>, yang menggunakan teknologi machine learning
            untuk menganalisis gambar fundus retina dan memberikan prediksi awal terhadap penyakit mata
            seperti <strong>Glaukoma</strong> dan <strong>Retinopati Diabetik</strong>.
        </p>
    </div>
    <div style='text-align: left; font-size: 16px; margin-top: 30px'>
        ...
        <ul style='list-style-position: inside; display: inline-block; text-align: left;'>
            <li>Upload gambar fundus retina.</li>
            <li>Deteksi penyakit secara otomatis.</li>
            <li>Hasil diagnosis disertai edukasi kesehatan mata.</li>
        </ul>
    ...
    if uploaded_file:
        ...
        with col2:
            ...
            ax.barh(class_names, probabilities.numpy() * 100)
            ax.set_xlim(0, 100)
            ax.set_xlabel("Probabilitas (%)")
            st.pyplot(fig)
            st.markdown("---")
            st.markdown(edukasi_penyakit[pred_label])
            laporan_text = f"""📄 Hasil Deteksi Fundus Mata
